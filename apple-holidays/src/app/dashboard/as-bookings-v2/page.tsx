...
function createdDate(b: ASBooking): string {
  const raw = b.created_iso ?? (typeof b.created_at === 'string' ? b.created_at : b.created_at?.date)
  if (!raw) return '—'
...
/** Created-date window for a named period filter. */
function rangeForFilter(filter: DateFilter): { from: string; to: string } {
  const now = new Date()
  if (filter === 'today') return { from: fmtDate(now), to: fmtDate(now) }
  if (filter === 'this_week') {
...
function ASBookingsV2Inner() {
  const router = useRouter()
  const [bookings, setBookings] = useState<ASBooking[]>([])
  const [total, setTotal] = useState(0)
  const [counts, setCounts] = useState<Counts>({ all: 0, confirmed: 0, unconfirmed: 0 })
  ...
  // Default = this week (created)
  const [dateFilter, setDateFilter] = useState<DateFilter>('this_week')
  const initialRange = useMemo(() => rangeForFilter('this_week'), [])
  ...
  const STATUS_TABS: { value: StatusFilter; label: string; icon: React.ComponentType<{ className?: string }>; count: number }[] = [
    { value: '2',   label: 'Confirmed',   icon: CheckCircle2, count: counts.confirmed },
    { value: '1',   label: 'Unconfirmed', icon: Clock,        count: counts.unconfirmed },
  ...
  }, [status, from, to, debouncedSearch, page])

  useEffect(() => { fetchBookings() }, [fetchBookings])

  // Country filter is applied client-side over the current page.
  ...
  function changeStatus(v: StatusFilter) { setStatus(v); setPage(1) }
  ...
  function openDetail(b: ASBooking) {
    ...
    router.push(`/dashboard/as-bookings-v2/${encodeURIComponent(String(b.id))}?${q.toString()}`)
  }
  ...
  return (
    <div>
      <Header
        ...
        subtitle="Confirmations from AppleSystem — filtered by created date"
        actions={
          <button
      ...
      <div className="p-4 sm:p-8 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <StatTile label="On this page" value={visible.length} icon={<Layers className="w-4 h-4" />} tone="slate" />
          <StatTile label="Confirmed" value={counts.confirmed} icon={<CheckCircle2 className="w-4 h-4" />} tone="emerald" />
          <StatTile label="Unconfirmed" value={counts.unconfirmed} icon={<Clock className="w-4 h-4" />} tone="amber" />
          <StatTile label="Matching filter" value={total} icon={<Cloud className="w-4 h-4" />} tone="brand" />
        </div>
        ...
        <Card className="p-4 space-y-4">
              ...
              {STATUS_TABS.map((tab) => {
                ...
                return (
                  ...
                  >
                    ...
                    {tab.label}
                    <span className={`ml-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-bold tabular-nums ${active ? 'bg-brand-100 text-brand-700' : 'bg-slate-200 text-slate-500'}`}>
                      {tab.count}
                    </span>
                  </button>
                )
          ...
          <div className="flex flex-col lg:flex-row gap-3">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <input type="date" value={from} onChange={(e) => { setFrom(e.target.value); setDateFilter('custom'); setPage(1) }} className="form-input text-sm py-1.5 w-40" />
              <span className="text-xs text-slate-400">→</span>
              <input type="date" value={to} onChange={(e) => { setTo(e.target.value); setDateFilter('custom'); setPage(1) }} className="form-input text-sm py-1.5 w-40" />
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            {dateFilter === 'custom' && (
              <span className="px-3 py-1 rounded-full text-xs font-medium border bg-brand-600 text-white border-brand-600">Custom</span>
            )}
          </div>
        ...
        {loading ? (
          ...
          <Card className="flex flex-col items-center justify-center h-56 text-slate-400">
            ...
            <p className="text-xs mt-1">Try widening the created-date window or switching status.</p>
          </Card>
        ) : view === 'grid' ? (
        ...
        {!loading && !error && total > 0 && (
          <div className="flex items-center justify-between gap-3 pt-1">
            <p className="text-xs text-slate-400">
              Page {page} of {pageCount} · {total} booking{total !== 1 ? 's' : ''} match
            </p>
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => setPage((p) => Math.max(1, p - 1))}
                disabled={page <= 1}
                className="flex items-center gap-1 px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed"
              >
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="min-w-0">
          <p className="font-mono font-bold text-slate-900 text-lg leading-none">{is ?? booking.quotation_no}</p>
      ...
      <div className="mt-4">
        <span className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-brand-700 bg-brand-50 border border-brand-100 group-hover:bg-brand-600 group-hover:text-white group-hover:border-brand-600 transition-colors">
          View Confirmation <ArrowRight className="w-3.5 h-3.5" />
        </span>
      </div>
    </button>
  )
          ...
          <tbody>
            {bookings.map((b) => (
              <tr key={b.id} className="cursor-pointer hover:bg-slate-50 transition-colors" onClick={() => onOpen(b)}>
                <td>
                  <span className="font-mono font-semibold text-slate-900">{isNumber(b) ?? b.quotation_no}</span>
                  <span className="block text-[11px] text-slate-400">quotation {b.quotation_no}</span>
                </td>
                ...
                <td className="text-xs text-slate-500 whitespace-nowrap">{createdDate(b)}</td>
...
export default function ASBookingsV2Page() {
  return (

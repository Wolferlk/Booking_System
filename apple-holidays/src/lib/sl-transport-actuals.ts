...
import {
  toBulkNo, toCostType, type ActualsStatus, type SettlementCostType,
...
/**
 * The vocabulary both systems share, defined once in a module with no database
 * in it.
 *
 * Re-exported here rather than imported-and-forgotten so every existing caller
 * of this module keeps working unchanged, while the register screen — which is
 * a browser bundle and must never pull `mysql2` in — can take the same values
 * from `sl-settlement-costs.ts` directly. See that file for why the split
 * exists.
 */
export {
  COST_TYPES, COST_TYPE_LABEL, toCostType, toBulkNo,
  type ActualsStatus, type SettlementCostType,
} from './sl-settlement-costs'

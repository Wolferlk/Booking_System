...
export interface ExtractedBooking {
  ...
  accommodations: { hotel: string; city: string; checkIn: string; checkOut: string; nights: number; roomType?: string; mealType?: string; ownArrangement?: boolean | null }[]
...
Return ONLY valid JSON matching this exact schema:
{
  ...
  "accommodations": [{ "hotel": "ONLY the actual hotel/resort/villa name (e.g. 'Novotel Hanoi', 'La Siesta Hotel'). NEVER include airport names, transfer directions, or route text. If the TC shows 'Airport to Hotel Name', the hotel field is just 'Hotel Name'.", "city": "city name", "checkIn": "YYYY-MM-DD", "checkOut": "YYYY-MM-DD", "nights": number, "roomType": "string or null", "mealType": "BB/HB/FB/null", "ownArrangement": "boolean — REQUIRED, never null. true when the GUEST or AGENT holds this reservation and we do not ('Own Arrangement', 'Own Arranged', 'Self Booked', 'Self Arranged', 'Own Basis', 'Guest Own', 'Booked by Guest', 'Direct Booking', accommodation not included, or no property named at all). false when we booked and hold it. NEVER infer it from the meal plan — 'Meals: Not included' is about breakfast, not about who booked the room." }],

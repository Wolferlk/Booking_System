...
function labelFor(country: string): string {
  return country === UNASSIGNED ? 'Unassigned' : countryLabel(country as never)
}
...
function countryWhere(countries: string[]): Prisma.BookingWhereInput | null {
  ...
  const named = countries.filter(c => c !== UNASSIGNED) as never[]
  const clauses: Prisma.BookingWhereInput[] = []
  if (named.length) clauses.push({ operationCountry: { in: named } })
  if (countries.includes(UNASSIGNED)) clauses.push({ operationCountry: null })
...
function toLine(b: RawBooking): BookingLine {
  const country = b.operationCountry ?? UNASSIGNED
  return {
...
async function collectCreated(w: ReportWindow, countries: string[], maxRows: number): Promise<CreatedSection> {
  ...
  const agentMap = new Map<string, { agent: string; bookings: number; pax: number }>()
  for (const l of lines) {
    const key = l.agent?.trim() || 'Unknown agent'
    const entry = agentMap.get(key) ?? { agent: key, bookings: 0, pax: 0 }
    entry.bookings += 1
    entry.pax += l.pax
  ...
  return {
    ...
    byAgent: Array.from(agentMap.values()).sort((a, b) => b.bookings - a.bookings).slice(0, 10),
...
async function collectComplaints(w: ReportWindow, countries: string[], maxRows: number): Promise<ComplaintsSection> {
  ...
  const bookings = refs.length
    ...
    : []
  const countryByRef = new Map(bookings.map(b => [b.bookingRef, b.operationCountry ?? UNASSIGNED]))
  ...
  const inScope = (c: ComplaintLine) => !countries.length || countries.includes(c.country)
...
async function collectUpcoming(w: ReportWindow, countries: string[], maxRows: number): Promise<UpcomingSection> {
  ...
  return {
    ...
    pax: lines.reduce((s, l) => s + l.pax, 0),
    channel: channelSplit(lines),
    next7: rows.filter(r => r.arrivalDate < in7).length,
    next30: rows.filter(r => r.arrivalDate < in30).length,
    beyond30: rows.filter(r => r.arrivalDate >= in30).length,
    byCountry: rollUpByCountry(lines),

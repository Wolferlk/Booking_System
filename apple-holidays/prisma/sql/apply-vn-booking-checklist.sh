#!/usr/bin/env bash
#
# Creates the `vn_booking_checklists` and `vn_booking_checklist_items` tables (Vietnam booking checklist).
#
...
#   bash prisma/sql/apply-vn-booking-checklist.sh
...
echo "Action : CREATE TABLE IF NOT EXISTS vn_booking_checklists, vn_booking_checklist_items  (additive only)"
...
DATABASE_URL="$URL" npx prisma db execute \
  --file prisma/sql/2026-09-23-vn-booking-checklist.sql \

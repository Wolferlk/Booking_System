#!/usr/bin/env bash
#
# Creates the `agenda_mc_details` table — the MC Report desk figures (package cost, KM,
# budget transfer, special request) per movement.
#
...
echo "Action : CREATE TABLE IF NOT EXISTS agenda_mc_details  (additive only)"
...
DATABASE_URL="$URL" npx prisma db execute \
  --file prisma/sql/2026-09-25-agenda-mc-details.sql \

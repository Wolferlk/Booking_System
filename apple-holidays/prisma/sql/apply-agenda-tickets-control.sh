#!/usr/bin/env bash
#
# Creates the `agenda_tickets_control` table — the MC Report's "Tickets Control"
# note on Vietnam movements.
#
...
#
# The SQL it runs is one CREATE TABLE IF NOT EXISTS. It does not ALTER, DROP or
# write to any existing table, and it is safe to run twice.
#
# Usage, from the apple-holidays/ directory:
#   bash prisma/sql/apply-agenda-tickets-control.sh
...
echo "Action : CREATE TABLE IF NOT EXISTS agenda_tickets_control  (additive only)"
...
DATABASE_URL="$URL" npx prisma db execute \
  --file prisma/sql/2026-09-25-agenda-tickets-control.sql \

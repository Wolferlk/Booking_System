#!/usr/bin/env bash
#
# Turns `agenda_items.serviceType` from an ENUM into VARCHAR(64) so the agenda
# can save service types the desk types in themselves.
#
...
#
# The SQL it runs is one ALTER TABLE ... MODIFY COLUMN. Every stored value is kept
# as the same string; nothing is dropped or deleted, and it is safe to run twice.
#
# Usage, from the apple-holidays/ directory:
#   bash prisma/sql/apply-agenda-custom-service-types.sh
#   bash prisma/sql/apply-agenda-custom-service-types.sh --check    # show the target, run nothing
...
echo "Action : ALTER agenda_items.serviceType ENUM -> VARCHAR(64)  (values kept)"
...
DATABASE_URL="$URL" npx prisma db execute \
  --file prisma/sql/2026-09-25-agenda-custom-service-types.sql \

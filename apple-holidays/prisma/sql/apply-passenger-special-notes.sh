#!/usr/bin/env bash
#
# Creates the `passenger_special_notes` table — the booking page's
# per-passenger "Special Note".
#
...
#   bash prisma/sql/apply-passenger-special-notes.sh
...
echo "Action : CREATE TABLE IF NOT EXISTS passenger_special_notes  (additive only)"
...
DATABASE_URL="$URL" npx prisma db execute \
  --file prisma/sql/2026-09-25-passenger-special-notes.sql \

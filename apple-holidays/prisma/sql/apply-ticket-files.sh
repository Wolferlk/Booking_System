#!/usr/bin/env bash
#
# Creates the `ticket_files` table, so one ticket can hold several files (one
# per guest on a group ticket).
#
...
#   bash prisma/sql/apply-ticket-files.sh
...
echo "Action : CREATE TABLE IF NOT EXISTS ticket_files  (additive only)"
...
DATABASE_URL="$URL" npx prisma db execute \
  --file prisma/sql/2026-09-25-ticket-files.sql \
